...
def close_connection(exception):
    connection =  getattr(g,'_conneciton', None)
...
def employer(employer_id):
    ...
    jobs = execute_sql('SELECT job.id, job.title, job.description, job.salary FROM job JOIN employer ON employer.id = job.employer_id =?',[employer_id])
...
def review(employer_id):
    if request.methods == 'POST':
        review =  request.from['review']
        rating =  request.from['rating']
